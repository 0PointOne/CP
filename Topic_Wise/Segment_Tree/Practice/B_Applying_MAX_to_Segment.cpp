...
void push_(int n, int l, int r){
    if(lazy[n] == 0)  return;
    tr[n] = max(tr[n], lazy[n]);
    if(r != l)   lazy[2*n] = max(lazy[2*n], lazy[n]),    lazy[2*n+1] = max(lazy[2*n+1], lazy[n]);
    lazy[n] = 0;
}
...
int mx_value(int n, int l, int r, int a, int b){
    push_(n, l, r);
    if(l > b || r < a) return INT_MIN;
    if(l >= a && r <= b) return tr[n];
    int mid = l + (r - l) / 2;
    return max(mx_value(2*n, l, mid, a, b), mx_value(2*n+1, mid+1, r, a, b));
}
