...
void push_(int n, int l, int r){
    if(lazy[n] == 1)  return;
    tr[n] = (1LL * tr[n] * lazy[n]) % MOD;
    if(r != l){
        lazy[2*n] = (1LL * lazy[2*n] * lazy[n]) % MOD;
        lazy[2*n+1] = (1LL * lazy[2*n+1] * lazy[n]) % MOD;
    }
    lazy[n] = 1;
}
...
int sum(int n, int l, int r, int a, int b){
    ...
    int mid = l + (r - l) / 2;
    return (sum(2*n, l, mid, a, b) + sum(2*n+1, mid+1, r, a, b) )% MOD;
}
